...
const chainWebpack = (chain, config) => {
	...
	if (config && config.automaticStart) {
		...
		chain.plugin("webpack-shell-plugin").use(WebpackShellPlugin, [
			{
				onBuildEnd: [
	...
	if (
		!(dev && tauriConfig.build.devPath.startsWith("http")) &&
		!tauriConfig.tauri.embeddedServer.active
	) {
		const TauriRequirePlugin = require("./plugins/tauri-require").plugin;
		chain.plugin("tauri-require").use(TauriRequirePlugin);
	}
};
